...
def Decision(Round, ActiveSbox, MatsuiRoundIndex, MatsuiCount, flag):
    ...
    # Open file
    file = open(
        f"Problem-Round{Round}-Active{ActiveSbox}.cnf", "w"
    )
    ...
    # Call solver cadical
    order = (
        f"~/Install/cadical/build/cadical Problem-Round{Round}-Active{ActiveSbox}.cnf > Round{Round}-Active{ActiveSbox}-solution.out"
    )
    os.system(order)
    # Extracting results
    order = (
        f"sed -n '/s SATISFIABLE/p' Round{Round}-Active{ActiveSbox}-solution.out > SatSolution.out"
    )
    os.system(order)
    order = (
        f"sed -n '/s UNSATISFIABLE/p' Round{Round}-Active{ActiveSbox}-solution.out > UnsatSolution.out"
    )
    os.system(order)
...
for totalround in range(SearchRoundStart, SearchRoundEnd):
    ...
    resultseq = (
        f"Round: {totalround}; Active S-box: {LinearActiveSbox[totalround]}; Runtime: {time_end - time_start}\n"
    )
    file.write(resultseq)
    file.close()
